...
import { MemberFormProps } from "@/app/types/memberForm";

export function MemberForm({nameForm, arrayMember, children}:MemberFormProps) {
  const [memberArray, setArrayMembers] = useState<Member[]>([
    { code: "", name: "", destinationChurch: "" },
  ]);
  const addMember = () => {
    setArrayMembers((prev) =>
      prev.length < 50 ? [...prev, { code: "", name: "", destinationChurch: "" }] : prev
    );
  };
  const removeMember = () => {
    setArrayMembers((prev) => prev.slice(0, -1));
  };
        ...
        <div key={index} className="col-span-12 grid grid-cols-12 relative">
          ...
          {memberArray.length > 1 ? (
            <button
              ...
              disabled={memberArray.length >= 50}
